...
parser = optparse.OptionParser(version = "1.00", description = "Generates local features", usage = "%prog [-opts] MOLFILES")
...
parser.add_option("-k", "--context", dest="context", type="string", default="all", help="Context size, <int> or <lb>-<ub>, e.g. '-k 3' or '-k 0-2'. Defaults to all possible contexts.")
parser.add_option("-x", "--exclude", dest="exclude", action="store_true", default=False, help="Exclude central atom info from feature")
parser.add_option("-u", "--unlimited", dest="unlimited", action="store_true", default=False, help="Enumerate contexts larger than the molecule")
...
if "all" == options.context:
	options.context = "0-1000"
...
# not features chosen -> choose atom features
if not options.ad and not options.bd and not options.wiener and not options.ring and not options.morgan:
	options.ad = True
...
def WriteResults(molfiles, features):
	# convert molfile into featfile
	for molfile in molfiles:
		
		if options.verbose:
			print "processing", molfile
		
		mol = re.findall(r'.*(C[0-9][0-9][0-9][0-9][0-9]).*', molfile)[0]
...
def ComputeFeatures():
	...
	for molfile in molfiles:
		...
		#  - atom_distribution_node_sumfunc function to sum up all incoming messages (containing labels)
		...
		if options.bd:
			AlgorithmB(g, bond_node_feat, bond_msg_feat, bond_distribution_node_sumfunc, bond_distribution_msg_sumfunc, verbose=options.debug, maxiter=max(context), limit_rounds=limit)
			ExtractFeatures(mol, g, "BOND_DISTRIBUTION", context, True)
		
		if options.ad and not options.exclude:
			AlgorithmB(g, atom_node_feat, msg_feat, atom_distribution_node_sumfunc, atom_distribution_msg_sumfunc, verbose=options.debug, maxiter=max(context), limit_rounds=limit)
			ExtractFeatures(mol, g, "ATOM_DISTRIBUTION", context, True)
		
		if options.ad and options.exclude:
			AlgorithmB(g, atom_node_feat, msg_feat, atom_distribution_exclude_node_sumfunc, atom_distribution_msg_sumfunc, verbose=options.debug, maxiter=max(context), limit_rounds=limit)
			ExtractFeatures(mol, g, "ATOM_DISTRIBUTION_EXCLUDE", context, True)
		
		if options.ring:
			AlgorithmB(g, distance_node_feat, msg_feat, ring_node_sumfunc, min_distance_msg_sumfunc, verbose=options.debug, maxiter=max(context), limit_rounds=limit)
		...
		if options.morgan:
			AlgorithmB(g, MorganNodeFeat(), MorganMsgFeat(), MorganNodeSumfunc(), MorganMsgSumfunc(), verbose=options.debug, maxiter=max(context), selfloops=True, limit_rounds=limit)
			ExtractFeatures(mol, g, "MORGAN", context, False, False)
		
		
#		print "Mol", mol, "computed"

		WriteResults([molfile], features)
