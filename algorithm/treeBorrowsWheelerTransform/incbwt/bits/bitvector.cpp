...
BitVector::BitVector(std::FILE * file) :
  rank_index(0), select_index(0)
{
    if (std::fread(&(this->size), sizeof(this->size), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to read file" << std::endl; std::exit(1); }
    if (std::fread(&(this->items), sizeof(this->items), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to read file" << std::endl; std::exit(1); }
    if (std::fread(&(this->number_of_blocks), sizeof(this->number_of_blocks), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to read file" << std::endl; std::exit(1); }
    if (std::fread(&(this->block_size), sizeof(this->block_size), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to read file" << std::endl; std::exit(1); }

  usint* array_buffer = new usint[this->block_size * this->number_of_blocks];
  if (std::fread(array_buffer, sizeof(usint), this->block_size * this->number_of_blocks, file) 
      != this->block_size * this->number_of_blocks)
  { std::cerr << "CSA::BitVector::BitVector(): unable to read file" << std::endl; std::exit(1); }
  this->array = array_buffer;
...
{
    if (std::fwrite(&(this->size), sizeof(this->size), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to write file" << std::endl; std::exit(1); }
    if (std::fwrite(&(this->items), sizeof(this->items), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to write file" << std::endl; std::exit(1); }
    if (std::fwrite(&(this->number_of_blocks), sizeof(this->number_of_blocks), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to write file" << std::endl; std::exit(1); }
    if (std::fwrite(&(this->block_size), sizeof(this->block_size), 1, file) != 1)
    { std::cerr << "CSA::BitVector::BitVector(): unable to write file" << std::endl; std::exit(1); }
    if (std::fwrite(this->array, sizeof(usint), this->block_size * this->number_of_blocks, file)
        != this->block_size * this->number_of_blocks)
    { std::cerr << "CSA::BitVector::BitVector(): unable to write file" << std::endl; std::exit(1); }
    this->samples->writeBuffer(file);
}
