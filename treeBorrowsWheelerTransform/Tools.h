...
/**
 * Simple header+version tag for index files
 */
#define TBWT_SAVEFILE_MSG "TBWT5"
...
public:
    ...
    static inline ulong GetField(ulong const *A, register unsigned len, register ulong index) 
    {
    ...
    static inline ulong GetVariableField(ulong const *A, register unsigned len, register ulong index) 
    {
    ...
    static inline void SetVariableField(ulong *A, register unsigned len, register ulong index, register ulong x) {
        ulong i=index/W, 
                    j=index-i*W;
        ulong mask = (j+len < W ? ~0lu << (j+len) : 0lu) 
                        | (W-j < W ? ~0lu >> (W-j) : 0lu);
        A[i] = (A[i] & mask) | x << j;
        if (j+len>W) {
            mask = ((~0lu) << (len+j-W));  
